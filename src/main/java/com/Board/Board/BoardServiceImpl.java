...
//        Page<Board> regIdPage = null;
        ...
        if(searchType.equals("regId")){
            return boardRepository.findByRegIdContaining(keyword, pageable).map(BoardForm::toDTO);
        }
        if(searchType.equals("title")){
            return boardRepository.findByTitleContaining(keyword, pageable).map(BoardForm::toDTO);
        }
        if(searchType.equals("content")){
            return boardRepository.findByContentContaining(keyword, pageable).map(BoardForm::toDTO);
        }
        return null;
    }
    ...
    public Page<BoardForm> searchBoardQuerydsl(SearchForm searchForm, Pageable pageable) {
        ...
        return boardRepository.search(searchForm, pageable)
                .map(this::searchFormtoBoardForm);
    }
